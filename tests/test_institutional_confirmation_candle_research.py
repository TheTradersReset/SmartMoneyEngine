"""Tests for institutional confirmation candle research."""
...
from src.research.institutional_confirmation_candle_research import (
    InstitutionalConfirmationCandleError,
    InstitutionalConfirmationCandleResearch,
    MOVE_THRESHOLDS,
    generate_institutional_confirmation_candle_report,
)
...
def test_analyze_move_produces_characteristics() -> None:
    engine = InstitutionalConfirmationCandleResearch(symbols=("NIFTY50",))
    ...
    record = engine._analyze_move("NIFTY50", frame, enriched, intel, candidate, "5M")
    assert record.feature_signature
    assert record.candle_characteristics["body_size_points"] >= 0
    assert record.signal_side == "BUY"
    assert "body_size_points" in record.candle_characteristics


def test_probability_matrix_and_scores() -> None:
    engine = InstitutionalConfirmationCandleResearch(symbols=("NIFTY50",))
    frame = _pipeline_frame()
    enriched = engine.context_builder.enrich(frame)
    enriched = engine.liquidity_map_engine._attach_calendar_levels(enriched)
    ...
    records = [
        engine._analyze_move(
            ...
            enriched,
            intel,
            _CheapMoveCandidate(60, 100 + index, "bullish", 120.0 + index * 15),
            "5M",
        )
        for index in range(12)
    ]
    power, _ = engine._predictive_power(records)
    scored = engine._apply_scores(records, power)
    matrix = engine._probability_matrix(scored)
    assert "BUY" in power
    assert isinstance(matrix, list)


def test_best_confirmation_candles_buy_sell() -> None:
    engine = InstitutionalConfirmationCandleResearch(symbols=("NIFTY50",))
    frame = _pipeline_frame()
    enriched = engine.context_builder.enrich(frame)
    enriched = engine.liquidity_map_engine._attach_calendar_levels(enriched)
    ...
    records = [
        engine._analyze_move(
            ...
            enriched,
            intel,
            _CheapMoveCandidate(60, 100 + index, "bullish" if index % 2 == 0 else "bearish", 120.0 + index * 10),
            "5M",
        )
        for index in range(6)
    ]
    buy = engine._best_confirmation_candles(records, "buy")
    sell = engine._best_confirmation_candles(records, "sell")
    assert buy or sell


def test_missing_metadata_raises() -> None:
    with pytest.raises(InstitutionalConfirmationCandleError):
        generate_institutional_confirmation_candle_report(filter_report_path=Path("missing.json"))
...
def test_full_report_if_metadata_exists() -> None:
    ...
    report = InstitutionalConfirmationCandleResearch(
        symbols=("NIFTY50",),
        timeframes=("5M",),
    ).run(metadata)
    assert report.total_moves_analyzed >= 0
    assert report.institutional_confirmation_candle_score
